...
type AccountPieProps = {
  accounts: AccountPieItem[];
  className?: string;
};
...
export function AccountPie({ accounts, className }: AccountPieProps) {
  const slices = accounts
    .map((account) => ({
      ...account,
      value: Math.max(account.value, 0),
    }))
    ...
    >
          ...
          <PieChart>
            ...
            <Tooltip
              ...
              content={({ active, payload }) => {
                ...
                const item = payload[0]?.payload as AccountPieItem | undefined;
                if (!item) return null;
                return (
                  <div className="rounded-md border border-border bg-popover px-2.5 py-1.5 text-xs text-popover-foreground shadow-sm">
                    ...
                    <p className="tabular-nums text-muted-foreground">
                      {item.display} · {item.sharePct.toFixed(item.sharePct % 1 === 0 ? 0 : 1)}%
                    </p>
                  </div>
                );
              }}
      ...
      <ul className="flex max-h-full min-h-0 flex-col justify-center gap-2 overflow-y-auto overscroll-contain pr-0.5">
        {accounts.map((account, index) => (
          <li key={account.id} className="flex items-start gap-2 text-xs">
            <span
